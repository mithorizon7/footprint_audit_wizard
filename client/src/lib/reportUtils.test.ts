...
describe("reportUtils scoring", () => {
  ...
  it("scores public exposure based on pages and people-search", () => {
    const good = getPublicExposureLevel(
      {
        ...baseResults,
        publicExposure: { ...baseResults.publicExposure, searchResultPagesWithPersonalInfo: 0, peopleSearchSitesFound: "no" },
      },
      false
    );
  ...
  it("scores trackers by count and flags unknown when missing", () => {
    ...
    const good = getTrackerLevel({ ...baseResults, trackers: { ...baseResults.trackers, trackerCount: 5 } }, false);
    const warning = getTrackerLevel({ ...baseResults, trackers: { ...baseResults.trackers, trackerCount: 20 } }, false);
    const critical = getTrackerLevel({ ...baseResults, trackers: { ...baseResults.trackers, trackerCount: 60 } }, false);
  ...
  it("scores cleanup based on cookie actions", () => {
    ...
    const warning = getCleanupLevel(
      {
        ...baseResults,
        cleanup: { ...baseResults.cleanup, cookiesCleared: "yes", thirdPartyCookiesBlockedOrLimited: "unsure" },
      },
      false
    );
    const good = getCleanupLevel(
      {
        ...baseResults,
        cleanup: { ...baseResults.cleanup, cookiesCleared: "yes", thirdPartyCookiesBlockedOrLimited: "yes" },
      },
      false
    );
    expect(critical).toBe("critical");
